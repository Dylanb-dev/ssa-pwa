...
function App() {
  ...
  const [selectedTimer, setSelectedTimer] = useState(TIMER_VALUES[0])
      ...
      <header className="App-header">
          ...
          <Modal isOpen={isOpen} onClose={onClose}>
            ...
            <ModalContent>
              ...
              <ModalCloseButton />
              <ModalBody>
                <Flex direction="column" id="capturedFrames">
        ...
        <Flex align="center" width="100%"><img src={logo} className="App-logo" alt="logo" /><Heading fontSize='6xl' colorScheme="blue">SSA</Heading></Flex>
        <Box borderWidth='1px' borderRadius='lg' overflow='hidden' width="100%" height={Math.min(480/2, VIEW_WIDTH/2)}>
          <video id="video-preview" autoPlay playsInline></video>
          <audio/>
          {isRecording ? <></> : <Text colorScheme="blue">Image preview here</Text>}
        </Box>
        <HStack {...group} p="4px">
          {options.map((value) => {
        ...
        {selectedTimer === TIMER_VALUES[1] && <NumberInput my="8px" step={5} size='lg' defaultValue={duration} min={10} max={1000} w="100%" color="blackAlpha.600" onChange={e => setDuration(e)}>
          <NumberInputField />
          <NumberInputStepper>
            <NumberIncrementStepper />
            <NumberDecrementStepper />
        ...
        {selectedTimer === TIMER_VALUES[2] && <NumberInput my="8px" step={5} size='lg' defaultValue={numberOfImages} min={10} max={1000} w="100%" color="blackAlpha.600" onChange={e => setNumberOfImages(e)}>
          <NumberInputField />
          <NumberInputStepper>
            <NumberIncrementStepper />
            <NumberDecrementStepper />
        ...
        {selectedTimer !== TIMER_VALUES[0] &&  <Flex my="8px" w="100%" align="center"><Switch alignItems="center"  size="md"/> <Text ml="8px" color="#1A202C" fontSize="md">Alarm when finished</Text></Flex>}
        <Flex my="8px" align="center" w="100%"><Switch alignItems="center" isChecked size="md"/> <Text ml="8px" color="#1A202C" fontSize="md">5 second timer before start</Text></Flex>
       
        {isRecording ? 
          <Button mt="5px" colorScheme="blue" variant='solid' onClick={(e) => {
            setIsRecording(false)
            stopStreamedVideo(e)
          }}>
            Stop Recording
          </Button> :
          <Button mt="5px" colorScheme="blue" variant='solid' isDisabled={isRecording} onClick={(e) => {
            setIsRecording(true)
            startRecording(e)
            dingSound.play()
          }}>
